    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div v-if="processedImage.url" class="p-6 w-full mb-8">
        <div class="bg-gray-800 rounded-lg p-6">
          <h3 class="text-xl font-bold text-white mb-4">Grayscale Conversion Complete!</h3>
      ...
      <div v-if="!processedImage.url" class="bg-gray-800 rounded-lg shadow-xl overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-700">
          <h1 class="text-2xl font-bold text-white">Convert to Grayscale</h1>
          <p class="mt-2 text-gray-300">Transform your color images into elegant black and white versions</p>
        </div>
        ...
        <form @submit.prevent="processImage" class="p-6 space-y-6">
          ...
          <!-- Grayscale Options -->
          <div class="bg-gray-700 rounded-lg p-4">
            <h3 class="text-white font-medium mb-3">Conversion Options</h3>
            <div class="space-y-3">
              <div>
                <label class="flex items-center">
                  <input 
                    type="radio" 
                    name="grayscaleMethod" 
                    value="standard" 
                    v-model="grayscaleMethod"
                    class="text-indigo-600 focus:ring-indigo-500 focus:ring-offset-gray-800"
                  />
                  <span class="ml-2 text-gray-300">Standard Grayscale</span>
                </label>
                <p class="text-xs text-gray-400 ml-6">Balanced conversion preserving overall brightness</p>
              </div>
              <div>
                <label class="flex items-center">
                  <input 
                ...
                <p class="text-xs text-gray-400 ml-6">Enhanced contrast for dramatic effect</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-700 rounded-lg p-4 flex items-center justify-between">
            ...
            <div class="text-right">
              <p class="text-white font-medium">Cost: 2 credits</p>
              <router-link 
          ...
          <div>
            <button 
              ...
              :disabled="isProcessing || !selectedFile || user.credits < 2"
            >
              <span v-if="!isProcessing">Convert to Grayscale</span>
...
export default {
  ...
  methods: {
    ...
    async processImage() {
      ...
      if (this.user.credits < 2) {
        this.error = 'You need at least 2 credits to convert to grayscale';
        return;
      }
      ...
      try {
        ...
        formData.append('method', this.grayscaleMethod);
        
        const response = await fetch(`${API_URL}/api/v1/create/grayscale/`, {
          method: 'POST',
          headers: {
    ...
    getCompletedImage(imageData) {
      ...
      // Emit event to parent to update credits (Grayscale conversion costs 2 credits)
      this.$emit('creditsUsed', 2);
    },
    
    downloadImage() {
      const url = this.processedImage.alternate_url || this.processedImage.url;
      const filename = 'grayscale-image.png';
