    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div v-if="!processedImage.url" class="bg-gray-800 rounded-lg shadow-xl overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-700">
          <h1 class="text-2xl font-bold text-white">Create Image Outline</h1>
          <p class="mt-2 text-gray-300">Generate clean line art and outlines from your images using edge detection</p>
        </div>
        ...
        <form @submit.prevent="processImage" class="p-6 space-y-6">
          ...
          <!-- Outline Options -->
          <div class="bg-gray-700 rounded-lg p-4">
            <h3 class="text-white font-medium mb-3">Outline Settings</h3>
            <div class="space-y-4">
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">
                ...
                <select 
                  v-model="edgeSensitivity"
                  class="w-full px-4 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
                  <option value="low">Low (Thick lines, less detail)</option>
                  <option value="medium">Medium (Balanced)</option>
                  <option value="high">High (Thin lines, more detail)</option>
                </select>
              </div>
              ...
              <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                  <label class="flex items-center p-3 bg-gray-600 rounded-lg cursor-pointer hover:bg-gray-500">
                    <input 
                      type="radio" 
                      name="lineStyle" 
                    ...
                    />
                    <span class="ml-2 text-gray-300">Black on White</span>
                  </label>
                  <label class="flex items-center p-3 bg-gray-600 rounded-lg cursor-pointer hover:bg-gray-500">
                    <input 
                      type="radio" 
                      ...
                      class="text-indigo-600 focus:ring-indigo-500"
                    />
                    <span class="ml-2 text-gray-300">White on Black</span>
                  </label>
                </div>
              </div>
            </div>
          ...
          <div class="bg-gray-700 rounded-lg p-4 flex items-center justify-between">
            ...
            <div class="text-right">
              <p class="text-white font-medium">Cost: 3 credits</p>
              <router-link 
          ...
          <div>
            <button 
              ...
              :disabled="isProcessing || !selectedFile || user.credits < 3"
            >
              <span v-if="!isProcessing">Create Outline</span>
              <span v-else>Creating Outline...</span>
            </button>
          </div>
        </form>
      </div>
  ...
  methods: {
    ...
    async processImage() {
      ...
      if (this.user.credits < 3) {
        this.error = 'You need at least 3 credits to create an outline';
        return;
      }
      ...
      try {
        ...
        formData.append('sensitivity', this.edgeSensitivity);
        formData.append('style', this.lineStyle);
        
        const response = await fetch(`${API_URL}/api/v1/create/outline/`, {
          method: 'POST',
          headers: {
    ...
    getCompletedImage(imageData) {
      ...
      // Emit event to parent to update credits (Outline creation costs 3 credits)
      this.$emit('creditsUsed', 3);
    },
    ...
    resetForm() {
      ...
      this.error = null;
      this.isProcessing = false;
      this.edgeSensitivity = 'medium';
      this.lineStyle = 'black_on_white';
    }
  },
